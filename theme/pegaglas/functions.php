<?php
/**
 * Pega Glas AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Pega Glas AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through pe_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'pegaglas', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'pegaglas' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'pegaglas' ) ) );

		register_block_pattern_category( 'pegaglas-sections', array( 'label' => __( 'Pega Glas AB sections', 'pegaglas' ) ) );
		register_block_pattern_category( 'pegaglas-pages', array( 'label' => __( 'Pega Glas AB page layouts', 'pegaglas' ) ) );
	}
);
