 ...
 * or photo. They call pe_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Pega Glas AB.
 *
 ...
 * @return string
 */
function pe_sample_note() {
	return pe_lead( 'ar_riktig' ) ? '' : ' <span class="pe-sample">(platshållare)</span>';
}
 ...
 * @return string File name for pe_lead_img().
 */
function pe_lead_bild( $i ) {
	$bilder = (array) pe_lead( 'bilder', array() );
 ...
 * @return string
 */
function pe_ring_label() {
	$namn = pe_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, pe_lead( 'telefon' ) ) : 'Ring ' . pe_lead( 'telefon' );
}
