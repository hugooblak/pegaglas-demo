...
add_filter(
	...
	function ( $exporters ) {
		$exporters['pegaglas-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'pegaglas-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'pegaglas-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'pegaglas-leads',
						'group_label' => __( 'Quote requests', 'pegaglas-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['pegaglas-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'pegaglas-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
