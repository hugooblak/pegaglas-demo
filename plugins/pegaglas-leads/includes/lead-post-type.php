...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'pegaglas-leads' ),
					'singular_name' => __( 'Lead', 'pegaglas-leads' ),
					'edit_item'     => __( 'Förfrågan', 'pegaglas-leads' ),
					'search_items'  => __( 'Search leads', 'pegaglas-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'pegaglas-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'pegaglas-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'pegaglas-leads' ),
			'npl_phone'    => __( 'Phone', 'pegaglas-leads' ),
			'npl_service'  => __( 'Needs', 'pegaglas-leads' ),
			'npl_timeline' => __( 'When', 'pegaglas-leads' ),
			'npl_zip'      => __( 'ZIP', 'pegaglas-leads' ),
			'npl_source'   => __( 'Source', 'pegaglas-leads' ),
			'npl_status'   => __( 'Status', 'pegaglas-leads' ),
			'date'         => __( 'Received', 'pegaglas-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'pegaglas-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'pegaglas-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'pegaglas-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'pegaglas-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'pegaglas-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'pegaglas-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'pegaglas-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'pegaglas-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'pegaglas-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'pegaglas-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'pegaglas-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'pegaglas-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
